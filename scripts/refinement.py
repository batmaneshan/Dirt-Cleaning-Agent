#!/usr/bin/env python
# encoding: utf-8
from checkbox_support import helpers
...
class Refine:
    ...
    def execute_action(self):
        ...
        # find what type of action is this
        this_action = "place"
        if type(action) == type([]):
            this_action = "move"
        elif len(action) == 2:
        ...
        # execute respective action from helper
        if this_action == "move":
            self.helper.execute_move_action(action)
        elif this_action == "pick":
            self.helper.execute_pick_action(action[0], action[1])
        elif this_action == "place":
            self.helper.execute_place_action(action[0], action[1], action[2])
    ...
    def refine_plan(self, plan_file, planner):
        ...
        with open(ROOT_PATH+'/objects.json', 'r') as file:
            f_json = json.load(file)

        # open file to get plan
        with open(plan_file) as f_plan:
            lines = f_plan.readlines()
        ...
        for line in lines:
            ...
            if action == "move":
                # move code
                obj = line.split(' ')[-1].split('_iloc)')[0]
                type = "bins"
                if obj.split('_')[0] == "book":
                    type = "books"
                goal_location = f_json[type][obj]["load_loc"]
                temp, current_state, checker  = self.get_path(current_state, goal_location)
                action_list.append(temp)

                # if there is any problem with returned actions, checker would be false indicating that no solution exists
                if checker is False:
                    return []
                ...
                # pick code
                obj = line.split(' ')[1]
                action_list.append((obj, current_state))

            elif action == "place":
                # place code
                obj1 = line.split(' ')[1]
                obj2 = line.split(' ')[2]
                action_list.append((obj1, obj2, current_state))
