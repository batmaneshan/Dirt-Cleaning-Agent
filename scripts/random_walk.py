...
class RandomWalker:
    ...
    def get_max_over_all_actions(self, row, col, V_Values):
        ...
        for action in self.actions.keys():
            ...
            for possible_action in possible_actions.keys():
                ...
                value_computed = value_computed + possible_actions[possible_action] * (
                        reward[row][col] + self.gamma * V_Values[next_state_row][next_state_col])
            ...
            for row in range(self.grid_dimension):
                for col in range(self.grid_dimension):
                    if ((row, col) in self.dirt_locations):
                        ...
                        for possible_action in possible_actions.keys():
                            next_state_row, next_state_col = self.get_next_cell_on_action((row, col), possible_action)
                            Final_V_Values[row][col] = Final_V_Values[row][col] + possible_actions[possible_action] * (
                                    reward[row][col] + self.gamma * initial_V_Values[next_state_row][
                                next_state_col])
    ...
    def main(self):
        ...
        bot_current_location, bot_current_orientation = (bot_current_state["robot"]["x"],
                                                         bot_current_state["robot"]["y"]), bot_current_state["robot"][
                                                            "orientation"]
        ...
        while (self.count_of_dirty_cells > 0):
            ...
            if (bot_current_location in self.dirt_locations):
                ...
                next_state, next_orientation = (next_bot_state["robot"]["x"], next_bot_state["robot"]["y"]), \
                                               next_bot_state["robot"]["orientation"]
                if (isSuccess == True):
                ...
                isSuccess, next_bot_state = self.execute_action(bot_current_location, bot_current_orientation,
                                                                best_action)
                next_state, next_orientation = (next_bot_state["robot"]["x"], next_bot_state["robot"]["y"]), next_bot_state["robot"]["orientation"]

            total_steps_of_cleaning_entire_grid = total_steps_of_cleaning_entire_grid + 1
            print("**Action : " + str(self.index_to_action_mapping[best_action]))
...
if __name__ == "__main__":
    ...
    RandomWalker(grid_dimension, dirt_locations, dirt_location_to_id_mapping)