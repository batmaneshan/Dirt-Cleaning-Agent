...
class Helper:
    ...
    def execute_suck_action(self, garbage_id, state):
        rospy.wait_for_service('execute_suck_action')
        try:
            suck_action = rospy.ServiceProxy('execute_suck_action', SuckActionMsg)
            response = suck_action(garbage_id, state.x, state.y, state.orientation)
